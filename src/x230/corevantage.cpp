...
void corevantage::displaySettings(int result) {
    ...
    if (result == 0) {
        ...
        textToDisplay(me_options, init_config->values[17], ui->meStateCombo);
        ...
        ui->gfxCombo->setCurrentIndex(gfx_map[init_config->values[19]]);

        //display slider values
        hexToSlider(init_config->values[1], ui->rebootSlider);
        hexToSlider(init_config->values[18], ui->meResetSlider);
        ...
        checkToDisplay(init_config->values[16], ui->functionsBox);
...
void corevantage::writeToFile(std::string out_file) {
    ...
    if(out_stream.is_open()) {
        ...
        checkToFile(out_stream, "f1_to_f12_as_primary = ", ui->functionsBox);
