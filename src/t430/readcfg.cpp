...
void readCfg::get_values() {
    ...
    if (curr_file.is_open() ) {
        ...
        while (getline(curr_file, curr_line)) {
            ...
            else if (values[counter - 1] == "only" || values[counter - 1] == "Only") {
                shrunken = shrunken.substr(0, last_space);
                int space_before = shrunken.find_last_of(' ');
                values[counter - 1] = shrunken.substr(space_before + 1, shrunken.size() - space_before);
            }
        }
    }
    curr_file.close();
