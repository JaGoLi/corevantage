    ...
    , ui(new Ui::corevantage)
{
    ...
    //gfxCard_map
    gfxCard_map["Integrated"] = 0;
    gfxCard_map["Discrete"] = 1;
    gfxCard_map["Graphics"] = 2;

    //backlight_map
    backlight_map["Both"] = 0;
    backlight_map["Keyboard"] = 1;
    backlight_map["Thinklight"] = 2;
    backlight_map["None"] = 3;
    ...
    //gfx_uma_size
    gfx_options << "32" << "64" << "96" << "128" << "160" << "192" << "224";
    ...
    //backlight
    backlight_options << "Both" << "Keyboard only" << "Thinklight only" << "None";
    ui->backlightCombo->addItems(backlight_options);

    //gfxCard
    gfxCard_options << "Integrated Only" << "Discrete Only" << "Dual Graphics";
    ui->gfxCardCombo->addItems(gfxCard_options);

    //usb_always_on
    usb_always_on << "Disable" << "AC and battery" << "AC only";
    ui->usbAOCombo->addItems(usb_always_on);
    ...
    //me_state
    me_options << "Normal" << "Disabled";
    ui->meStateCombo->addItems(me_options);
    ...
    connect(ui->meResetSlider, &QSlider::valueChanged, this, &corevantage::setMeValue);
...
void corevantage::setMeValue() {
     ui->meResetValue->setText(QString::number(ui->meResetSlider->value()));
 }
...
void corevantage::displaySettings(int result) {
    ...
    if (result == 0) {
        ...
        textToDisplay(sata_options, init_config->values[14], ui->sataCombo);
        textToDisplay(me_options, init_config->values[16], ui->meStateCombo);
        ui->usbAOCombo->setCurrentIndex(usbAO_map[init_config->values[13]]);
        ui->backlightCombo->setCurrentIndex(backlight_map[init_config->values[15]]);
        ui->gfxCombo->setCurrentIndex(gfx_map[init_config->values[18]]);
        ui->gfxCardCombo->setCurrentIndex(gfxCard_map[init_config->values[19]]);

        //display slider values
        hexToSlider(init_config->values[1], ui->rebootSlider);
        hexToSlider(init_config->values[17], ui->meResetSlider);
        hexToSlider(init_config->values[20], ui->volumeSlider);

        //display checkbox states
        checkToDisplay(init_config->values[3], ui->nmiBox);
        checkToDisplay(init_config->values[4], ui->acAttachBox);
        checkToDisplay(init_config->values[6], ui->btBox);
        checkToDisplay(init_config->values[7], ui->wwanBox);
        checkToDisplay(init_config->values[8], ui->touchpadBox);
        checkToDisplay(init_config->values[9], ui->wlanBox);
        checkToDisplay(init_config->values[10], ui->trackpointBox);
...
void corevantage::writeToFile(std::string out_file) {
    ...
    if(out_stream.is_open()) {
        ...
        checkToFile(out_stream, "nmi = ", ui->nmiBox);
        checkToFile(out_stream, "power_on_after_fail = ", ui->acAttachBox);
        ...
        checkToFile(out_stream, "touchpad = ", ui->touchpadBox);
        checkToFile(out_stream, "wlan = ", ui->wlanBox);
        checkToFile(out_stream, "trackpoint = ", ui->trackpointBox);
        ...
        comboToFile(out_stream, "backlight = ", ui->backlightCombo);
        ...
        comboToFile(out_stream, "hybrid_graphics_mode = ", ui->gfxCardCombo);
